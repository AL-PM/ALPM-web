...
function MyCodeGroup(){
    ...
    const [myCodeGroupInfo, setMyCodeGroupInfo] = useState();
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                setMyCodeGroupInfo(response.data.content);
    ...
    if (!searchResult || !myCodeGroupInfo){
        return (
            <div id="MyCodeGroup">
    ...
    console.log(searchResult, myCodeGroupInfo);

    return(
        <div id="MyCodeGroup">
            <MainMenuBar page={"MyPage"} />
            <MyPageMenuBar MyPage={"3"}/>
            <CodeGroupSearchResult searchData={myCodeGroupInfo} bodyHeight={"60vh"} />
            <MyCodeGroupNew />
        </div>
    )
}
